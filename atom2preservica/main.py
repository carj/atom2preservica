...
def create_parent_series(atom_client: AccessToMemory, entity: EntityAPI, slug: str, security_tag: str, parent_collection: Folder = None):
    ...
    parent_slug: str = item.get('parent', None)
...
def get_folder(entity: EntityAPI, atom_record, atom_client: AccessToMemory, security_tag: str, parent_collection: Folder):
    ...
    Find the parent for the record which is foing to be linked, create the parent series of folders if they do not exist
...
def init(args):
    ...
    if cmd_line["atom_api_key"] is not None:
        atom_client = AccessToMemory(api_key=cmd_line["atom_api_key"],  server=cmd_line["atom_server"])

    else:
        atom_client = AccessToMemory(username=cmd_line["atom_user"], password=cmd_line["atom_password"], server=cmd_line["atom_server"])
